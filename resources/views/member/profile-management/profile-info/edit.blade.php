								...
								<div class="col-sm-8">
									<div class="form-group text-right">
										...
										<button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
											<a href="{{route('member.profile-management.profile-info.profile') }}">Back</a>
										</button>
									</div>
								</div>
							</div>
	...
	$(document).ready(function(){
		$('#submitForm').validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					url : "{{route('member.profile-management.profile-info.update')}}",
					data : formInfo,
					type : "POST",
					processData: false,
					contentType: false,
		...
		jQuery.validator.addClassRules({
			...
			'email' : {
				...
				email : true,
				remote: {
					url: "{{route('member.profile-management.profile-info.duplicate-email-check')}}",
					type: "GET",
					data: {
			...
			'mobile_no' : {
				...
				// maxlength:5,
				remote: {
					url: "{{route('member.profile-management.profile-info.duplicate-mobile_no-check')}}",
					type: "GET",
					data: {
