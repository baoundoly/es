...
class ProfileController extends Controller
{
    ...
    {
        $exist = Admin::where('id','!=',auth()->user()->id)->where('email',$request->email)->first();
        if($exist){
    ...
    {
        $exist = Admin::where('id','!=',auth()->user()->id)->where('mobile_no',$request->mobile_no)->first();
        if($exist){
    ...
    public function update(Request $request)
    {
        $validator = Validator::make($request->all(), 
            [
                ...
                'mobile_no'     => ['required','digits:11','unique:admins,mobile_no,'.auth()->user()->id],
